<template> 
  <div>
    ...
    <h1 class="mb-8 font-bold text-3xl">Learning</h1>
    <!-- view mobile -->
    ...
    <div class="mb-6 flex justify-between items-center" v-if="isMobile()">
      ...
      <inertia-link v-if="$page.auth.user.usertype_id==1||$page.auth.user.usertype_id==4" class="btn-indigo" :href="route('list_approve_course_student')">
        Approve Join 
      </inertia-link>
    </div>
    <!-- tampilan non-mobile -->
          ...
          <article class="overflow-hidden bg-white rounded-lg shadow-lg">
            ...
            <footer class="flex items-center justify-between leading-none px-4 pb-4">
              <div class="pt-1">
                ...
                <a :href="route('course-modules.getCourseUnitNotActive', course.id)" v-if="role.is_teacher" class="flex-1 mx-1">
                  <span class="inline-block bg-red-200 rounded-full px-3 py-2 text-sm font-normal text-gray-700 mr-2">Learning Unit Not Active</span>
                </a>
                ...
                <div v-else >
                  <div v-if="course.course.is_active == 1">
                    ...
                    <a :href="route('course_users_show', course.id)" data-toggle="tooltip" title="Daftar Murid" class="flex-1 mx-1">
                      <span class="inline-block bg-yellow-200 rounded-full px-3 py-2 text-sm font-normal text-gray-700 mr-2"> <i class="fa fa-users" aria-hidden="true"></i> List Students</span>
                    </a>
                  ...
                  <div v-else-if="course.course.is_active == 0">
                    <span  class="btn-indigo bg-red-500"
                      tabindex="-1" v-on:click="toggleModal(course.id, 'Cancel')">
...
export default {
  ...
  data() {
    return {
      role: {
        ...
        is_admin: this.$page.auth.user.usertype_id == 1 || this.$page.auth.user.usertype_id == 4,
  ...
  methods: {
    ...
    toggleModal: function(course_id = null, desc = ""){
      ...
      this.modal.descModal = 'Are you sure you want  '+ desc + ' of this Learning? '
