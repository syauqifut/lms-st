<template>
  <div>
    <div class="bg-gray-100 font-bold rounded shadow-xl py-5 px-5 w-full mb-10">
      ...
      <div class="mb-6 flex justify-between items-center" v-if="isMobile()">
        <div class="w-full h-12 py-2 px-5">
          ...
          <v-select label="fullname" :reduce="children => children.id" v-model="form.child_id" :options="children"></v-select>
        </div>
      </div>
      <div class="mb-6 flex justify-between items-center" v-if="isMobile()">
    ...
    <div class="bg-white rounded shadow overflow-x-auto">
      <br>
      <div>
        <h1 v-if="presences.length !== 0" class="mb-8 font-bold text-3xl">&nbsp;&nbsp;Attends= {{datasiswa.hadir}}; Lates= {{datasiswa.terlambat}}; Sicks= {{datasiswa.sakit}}; Permits= {{datasiswa.izin}}; Skips= {{datasiswa.alpha}};</h1>
      </div>
...
export default {
  ...
  methods: {
    cari() {
      ...
      this.$inertia.replace(this.route('absensi.absen_ortu',
        {
          tgl_mulai: tgl_mulai,
          tgl_akhir: tgl_akhir,
          id_anak: id_anak,
