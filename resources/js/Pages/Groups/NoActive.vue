<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Groups</h1>
    <!-- view mobile -->
    ...
    <div class="mb-6 flex justify-between items-center" v-if="isMobile()">
      <inertia-link class="btn-indigo" :href="route('groups.create')">
        <span>Create</span>
      </inertia-link>
    ...
    <div class="mb-6 flex justify-between items-center" v-if="!isMobile()">
      ...
      <div>
        <inertia-link class="btn-indigo" :href="route('groups.create')">
          <span>Create</span>
          <span class="hidden md:inline">Class</span>
        </inertia-link>
        <inertia-link class="btn-indigo" :href="route('list-approve-student')">
          <span>Approve Join</span>
        </inertia-link>
    ...
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        <tr class="text-left font-bold">
          ...
          <th class="px-6 pt-6 pb-4">Teachers</th>
        ...
        <tr v-for="group in groups.data" :key="group.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center focus:text-indigo-500" :href="route('group_users.show', group.id)">
              {{ group.classes + ( group.huruf ? " - " + group.huruf : '') }}
              <icon v-if="group.deleted_at" name="trash" class="flex-shrink-0 w-3 h-3 fill-gray-400 ml-2" />
            </inertia-link>
          </td>
          <td class="border-t">
          ...
          <td class="border-t">
            <div v-clipboard="group.code" v-clipboard:success="clipboardSuccessHandler" class="relative hover-trigger w-full">
              <span :id="group.code">
                {{ group.code }}
              </span>
          ...
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center" :href="route('group_users.show', group.id)" tabindex="-1">
              {{ group.academicterms === 1 ? 'Odd' : "Even" }}
            </inertia-link>
          </td>
          <td class="border-t">
            <inertia-link class="px-6 py-4 flex items-center" :href="route('group_users.show', group.id)" tabindex="-1">
          ...
          <td class="border-t w-px">
            <div>
              <div v-if="group.mainteacherr.id == $page.auth.user.id || $page.auth.user.usertype_id == 1" class="inline-flex">
                <a :href="route('groups.edit', group.id)" class="flex-1 mx-1">
...
export default {
  metaInfo: { title: 'Groups Not Active' },
  layout: Layout,
  components: {
    Icon,
    Pagination,
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('groups.groups_non_active', Object.keys(query).length ? query 
        : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
