<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      ...
      <span class="text-indigo-400 font-medium">/</span> Create Head Master
    </h1>
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      <form @submit.prevent="submit">
        ...
        <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
          ...
          >Create and Add new Head Master</span>
          <loading-button
            ref="btnsubmit"
            :loading="sending"
            class="btn-indigo"
            type="submit"
          >Create Head Master</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  metaInfo: { title: "Create Head Master" },
  layout: Layout,
  components: {
    LoadingButton,
    SelectInput,
  ...
  methods: {
    ...
    submit() {
      this.sending = true;
      this.$inertia.post(this.route("headmasters.store"), this.form).then(() => {
        this.sending = false;
        this.form.name = null;
