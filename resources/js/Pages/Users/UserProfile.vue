<template>
  <div>
    ...
    <div class="mb-8">
      <inertia-link class="btn-indigo" :href="route('users.profiles', user.id)">Profile Detail</inertia-link>
        ...
        <div class="container mx-auto pr-6 pb-6">
          <div class="flex flex-row flex-wrap -mx-2">
            <div class="w-full h-32 md:h-64 mb-4 sm:mb-0 px-2">
              ...
              <a v-if="user.photo_path" class="block w-full h-full bg-grey-dark bg-no-repeat bg-center bg-cover" href="#" title="Link" :style="'background-image: url('+ user.photo_path +');background-size:contain;'">
              </a>
              <a v-else class="block w-full h-full bg-grey-dark bg-no-repeat bg-center bg-cover" href="#" title="Link" :style="'background-image: url(/images/default_photo.png);background-size:contain;'">
              </a>
                <!-- {{ user.photo_path }} -->
                <!-- <img v-if="user.photo_path" class="block w-8 h-8 rounded-full ml-4" :src="user.photo_path" alt="profile picture user">
                <img v-else class="block w-8 h-8 rounded-full ml-4" src="/images/default_photo.png" alt="profile picture"> -->
            </div>
          </div>
        </div>
        </div>
      ...
      <div class=" flex-1 bg-white rounded shadow overflow-hidden max-w-3xl">
        <form @submit.prevent="submit">
          <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
            ...
            <text-input v-model="form.username" :errors="$page.errors.username" class="pr-6 pb-8 w-full lg:w-1/2" readonly label="Username" />
            ...
            <file-input name="photo_path" v-model="form.photo_path" :errors="$page.errors.photo_path" class="pr-6 pb-8 w-full lg:w-1/2" type="file" accept="image/*" label="Photo" />
            ...
            <select-input v-model="form.usertype_id" :disabled="!form.is_admin" :errors="$page.errors.usertype_id" class="pr-6 pb-8 w-full lg:w-1/2" label="User Type">
              <option :value="null"></option>
              <option v-for="t in listTypes" :key="t.id" :value="t.id">{{t.name}}</option>
            </select-input>
           <div class="pr-6 pb-8 w-full lg:w-1/2" v-if="user.usertype_id=='2'" :disabled="!form.is_admin">
          ...
          <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center justify-between">
            <div class="flex items-center">
              <inertia-link v-if="form.usertype_id === 5 && form.is_admin" class="btn-indigo mr-3" :href="route('users.create',[2, user.id])">Add Anak</inertia-link>
              <inertia-link v-else-if="form.usertype_id === 2 && form.parent_id === null && form.is_admin" class="btn-indigo mr-3" :href="route('users.create',[5, user.id])">Add Parent</inertia-link>
    ...
    &nbsp;&nbsp;
      <div v-if="user.usertype_id!='2'">
        <h1 class="mb-8 font-bold text-3xl">Children</h1>
        <div class="bg-white rounded shadow overflow-x-auto">
          <table class="w-full whitespace-no-wrap">
            ...
            >
                <td class="border-t">
                            <inertia-link class="px-6 py-4 flex items-center focus:text-indigo-500" :href="route('users.userprofile', userC.id)">
                          ...
                          <td class="border-t">
                            <inertia-link class="px-6 py-4 flex items-center" :href="route('users.userprofile', userC.id)" tabindex="-1">
                              {{ userC.email }}
                            </inertia-link>
                          </td>
                          <td class="border-t">
                          ...
                          <td class="border-t w-px">
                            <inertia-link class="px-4 flex items-center" :href="route('users.userprofile', userC.id)" tabindex="-1">
